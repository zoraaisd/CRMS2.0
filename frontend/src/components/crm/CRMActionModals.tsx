...
export function DeleteModal({ open, onClose }: BaseModalProps) {
  return (
    <CRMModalBase
      open={open}
      title="Delete Record"
      ...
      footer={
        <>
          <button onClick={onClose} className="rounded-lg border border-slate-300 px-3 py-2 text-sm">Cancel</button>
          <button onClick={onClose} className="rounded-lg bg-rose-600 px-3 py-2 text-sm text-white hover:bg-rose-700">Delete</button>
        </>
      }
    >
      <p className="text-sm text-slate-700">Are you sure you want to delete this record? This action cannot be undone.</p>
    </CRMModalBase>
  );
}
