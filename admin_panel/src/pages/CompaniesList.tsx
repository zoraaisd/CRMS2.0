...
export default function CompaniesList() {
    ...
    const [formData, setFormData] = useState({
        company_name: '', company_email: '', contact_person: '', subscription_plan: 'Basic'
    });
    ...
    const handleCreateCompany = async (e: any) => {
        ...
        try {
            ...
            alert(err?.response?.data?.error || "Failed to provision database.");
        } finally {
    ...
    const regenerateCredentials = async (id: number) => {
        if (confirm("Generate a new secure superuser credential for this tenant?")) {
            const res = await api.post(`admin/companies/${id}/generate-credential`);
    ...
    const removeCompany = async (id: number, name: string) => {
        if (confirm(`Remove ${name} from the client list? This cannot be undone.`)) {
            try {
                ...
                alert(err?.response?.data?.error || "Failed to delete company and tenant database.");
            }
        }
    };
                            ...
                            companies.map(c => (
                                <tr key={c.id} className="hover:bg-slate-50">
                                    <td className="py-4 px-6">
                                        <Link to={`/companies/${c.id}`} className="font-semibold text-slate-800 hover:text-blue-600">
                                            {c.company_name}
                                        </Link>
                                        <div className="text-sm text-slate-500">{c.company_email}</div>
                                    </td>
                                    <td className="py-4 px-6 text-slate-600">{c.contact_person}</td>
                                    <td className="py-4 px-6">
                                        <span className="font-mono text-sm bg-slate-100 text-slate-600 px-2 py-1 rounded">{c.db_name}</span>
                                    </td>
                                    <td className="py-4 px-6 text-slate-600">{c.subscription_plan}</td>
                                    <td className="py-4 px-6">
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${c.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                            {c.status}
                                        </span>
                                    </td>
                                    <td className="py-4 px-6">
                                        <div className="flex justify-end items-center gap-2">
                                        <button onClick={() => regenerateCredentials(c.id)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Regenerate Credentials">
                                            <Key className="w-4 h-4" />
                                        </button>
                                        <button
                                            onClick={() => removeCompany(c.id, c.company_name)}
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
                        ...
                        {newCredentials ? (
                            <div className="p-6">
                                ...
                                <button onClick={() => { setShowModal(false); setNewCredentials(null); }} className="w-full bg-slate-800 text-white rounded-lg py-2">Close</button>
                            </div>
                        ) : (
